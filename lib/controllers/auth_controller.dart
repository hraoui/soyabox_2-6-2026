...
class AuthController extends GetxController {
  ...
  Future<bool> loginUser(String identifier, String password) async {
    ...
    try {
      ...
      if (onlineUser != null) {
        // ✅ Vérifier que l'utilisateur appartient au restaurant importé
        if (!_isUserAllowedForCurrentRestaurant(onlineUser)) {
          throw Exception(
            'Accès refusé : Cet utilisateur appartient à un autre restaurant.',
          );
        }
        ...
        if (Get.isRegistered<UserController>()) {
          await Get.find<UserController>().fetchAllUsers(
            restaurantId: onlineUser.restaurantId,
          );
        }
        // Start sync only after successful login
        if (Get.isRegistered<SyncController>()) {
          Get.find<SyncController>().startSyncAfterLogin();
        }
        // Initialize UserController after login
        if (Get.isRegistered<UserController>()) {
          Get.find<UserController>().initAfterLogin();
        }
        // Initialize DeliveryController after login
        if (Get.isRegistered<DeliveryController>()) {
          Get.find<DeliveryController>().initAfterLogin();
        }

        // Redirect based on role
        if (onlineUser.role == 'cashier') {
  ...
  bool _isUserAllowedForCurrentRestaurant(User user) {
    ...
    // (le premier utilisateur pourra importer un restaurant)
    if (importedRestaurantId == null) {
      appLogger.w(
        '⚠️ [AUTH] Aucun restaurant importé, connexion autorisée (première installation)',
      );
      return true;
    }
    ...
    if (userRestaurantId == null || userRestaurantId <= 0) {
      ...
      // Utilisateur sans restaurant (hors superadmin) → accès refusé
      return false;
    }

    if (userRestaurantId != importedRestaurantId) {
      appLogger.e(
        '❌ [AUTH] Accès refusé: utilisateur restaurant=$userRestaurantId, importé=$importedRestaurantId',
      );
      return false;
    }

    appLogger.d(
      '✅ [AUTH] Accès autorisé: utilisateur restaurant=$userRestaurantId',
    );
    return true;
  }
  ...
  Future<User?> _tryOnlineLogin({
    ...
    try {
      ...
      if (phoneForApi == null) {
        appLogger.w(
          'No phone number found for login',
      ...
      if (response.statusCode != 200 && response.statusCode != 201) {
        ...
        appLogger.d('API login response body: ${response.body}');
        return null;
      }
      ...
      await AuthSessionService.instance.saveSession(
        ...
        email: resolvedEmail,
        authMethod: 'password',
        credentialIdentifier: identifier.trim().toLowerCase(),
  ...
  String _extractToken(Map body) {
    return extractAuthTokenFromApiBody(body);
  }
  ...
  Future<_PinLoginResult?> _tryOnlinePinLogin(String pin) async {
    try {
      ...
      if (remoteUserData.isEmpty) {
        return null;
      }

      final user = await _upsertUserFromRemotePin(
        pin: pin,
        data: remoteUserData,
      );
  ...
  Future<void> _completePinLogin({
    ...
    if (Get.isRegistered<UserController>()) {
      await Get.find<UserController>().initAfterLogin();
    }
    if (Get.isRegistered<DeliveryController>()) {
      await Get.find<DeliveryController>().initAfterLogin();
    }
  }
  ...
  Future<User> _loginWithPin({
    ...
    if (normalizedPin.isEmpty) {
      throw Exception('Code PIN requis');
    }

    User? user = await DatabaseService.getUserByPin(
      normalizedPin,
    ...
    if (onlineResult != null) {
      ...
      if (allowedRoles.contains(onlineRole) || user == null) {
    ...
    if (!allowedRoles.contains(normalizedRole)) {
      throw Exception(roleErrorMessage);
    }
    if (!user.isActive) {
    ...
    final token = onlineResult != null && _isSameUser(user, onlineResult.user)
        ? onlineResult.token
        : null;
  ...
  // Vérifications de permissions pour le rôle caissier
  ...
  Future<bool> loginAdminWithPin(String email, String pin) async {
    ...
    try {
      ...
      // Verify PIN
      if (user.pinCode == null || user.pinCode != pin) {
        throw Exception('Code PIN incorrect');
      }
  ...
  Future<bool> loginSuperAdminWithPin(String pin) async {
    ...
    try {
      ...
      appLogger.e(
        'Admin/Superadmin PIN login error',
        error: e,
        stackTrace: stackTrace,
      );
      rethrow;
