...
class _AdminPinLoginScreenState extends State<AdminPinLoginScreen> {
  ...
  final FocusNode _badgeListenerFocusNode = FocusNode(debugLabel: 'admin_badge_listener');
  ...
  void _onKeyTap(String char) {
    setState(() => _pinController.text += char);
  ...
  void _onBackspace() {
    ...
    setState(() => _pinController.text = text.substring(0, text.length - 1));
    _badgeListenerFocusNode.requestFocus();
  }
  ...
  // ✅ CORRIGÉ — utilise loginSuperAdminWithBadge au lieu de loginSuperAdminWithPin
  Future<void> _commitBadgeScan() async {
    ...
    try {
      ...
      if (!success || auth.currentUser == null) throw Exception('Badge refusé');
  ...
  Future<void> _handlePinLogin() async {
    ...
    try {
      ...
      final auth = Get.find<AuthController>();
      final success = await auth.loginSuperAdminWithPin(pin);
      final role = auth.currentRole?.trim().toLowerCase();
      if (!success || auth.currentUser == null) throw Exception('Échec de l\'authentification');
      if (role != 'admin' && role != 'superadmin') throw Exception('Accès réservé aux administrateurs');
      if (mounted) Get.offAllNamed('/financial-dashboard');
    } catch (e) {
      if (mounted) {
        Get.snackbar(
    ...
    return Column(
      children: [
        Container(
          ...
          child: ClipOval(
            child: Image.asset('assets/branding/app_icon.png', fit: BoxFit.contain),
          ),
        ),
        ...
        Text(
          ...
          style: TextStyle(fontSize: 13, color: Colors.white.withOpacity(0.65), letterSpacing: 1.5),
        ),
      ],
    );
  }
        ...
        if (wide) {
          return Row(
            ...
            children: [
              ...
              Expanded(flex: 6, child: _keyboardCard()),
            ],
          );
        }
        return Column(children: [_formCard(), const SizedBox(height: 16), _keyboardCard()]);
      },
    );
  }
          ...
          Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.lock_outline, color: Color(0xFFFF5252), size: 20),
              ),
            ...
            builder: (_, __, ___) {
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      Icon(txt.isEmpty ? Icons.badge : Icons.lock_outline, color: const Color(0xFFFF5252)),
                      const SizedBox(width: 10),
                      Expanded(
                        child: Text(
                          txt.isEmpty
                          ...
                          style: TextStyle(
                            color: txt.isEmpty ? Colors.white.withOpacity(0.4) : Colors.white,
  ...
  Widget _keyboardCard() {
    return _cardShell(
      child: _InlineKeyboard(
        ...
        onToggleMode: () => setState(() => _isNumericMode = !_isNumericMode),
        onToggleCase: () => setState(() => _isUpperCase = !_isUpperCase),
      ),
    );
  }
}
...
class _InlineKeyboard extends StatelessWidget {
  ...
  static const _row2 = ['Q','S','D','F','G','H','J','K','L'];
  static const _row3 = ['W','X','C','V','B','N','M'];
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Row(
          children: [
            Container(
              ...
              child: Icon(
                isNumericMode ? Icons.dialpad : Icons.keyboard_alt_outlined,
            ...
            const SizedBox(width: 10),
            Text(
              isNumericMode ? 'Pavé numérique' : 'Clavier',
        ...
        const SizedBox(height: 14),
        Divider(color: _KB.divider, height: 1),
        const SizedBox(height: 12),
        isNumericMode ? _buildNumpad() : _buildAlpha(),
      ],
    );
  }
  ...
  Widget _numKey(String d) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Center(
          child: Text(d, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Colors.white)),
        ),
      ),
    );
  }
  ...
  Widget _buildAlpha() {
    final row1 = _row1.map((e) => isUpperCase ? e.toUpperCase() : e.toLowerCase()).toList();
    final row2 = _row2.map((e) => isUpperCase ? e.toUpperCase() : e.toLowerCase()).toList();
    final row3 = _row3.map((e) => isUpperCase ? e.toUpperCase() : e.toLowerCase()).toList();
          ...
          children: [
            GestureDetector(
              ...
              child: Container(
                ...
                width: 50,
                decoration: BoxDecoration(
                  color: isUpperCase ? const Color(0xFFD32F2F) : _KB.keyBg,
                  borderRadius: BorderRadius.circular(7),
                  border: Border.all(color: isUpperCase ? const Color(0xFFFF5252) : _KB.keyBorder),
                ),
                child: const Center(child: Icon(Icons.keyboard_capslock, color: Colors.white, size: 18)),
              ),
            ),
            const SizedBox(width: 3),
            Expanded(child: _letterRow(row3)),
          ],
        ),
  ...
  Widget _letterRow(List<String> letters) {
    return Row(
      children: letters.expand((l) => [
        ...
        if (l != letters.last) const SizedBox(width: 3),
      ]).toList(),
    );
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: _KB.letterBg,
          borderRadius: BorderRadius.circular(7),
          border: Border.all(color: _KB.letterBorder),
        ),
        child: Center(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: _KB.keyBg,
          borderRadius: BorderRadius.circular(7),
          border: Border.all(color: _KB.keyBorder),
        ),
        child: Center(
          child: Text(
            d,
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: _KB.deleteBg,
          borderRadius: BorderRadius.circular(10),
          border: Border.all(color: _KB.deleteBorder),
        ),
        child: const Center(child: Icon(Icons.backspace_outlined, color: Color(0xFFFF5252))),
      ),
    );
  }
      ...
      children: [
        Expanded(
          child: GestureDetector(
            ...
            child: Container(
              height: 44,
              decoration: BoxDecoration(color: _KB.clearBg, borderRadius: BorderRadius.circular(10)),
              child: const Center(child: Text('Effacer', style: TextStyle(color: Colors.white))),
            ),
          ),
        ),
        ...
        if (!isNumericMode) ...[
          GestureDetector(
            ...
            child: Container(
              height: 44,
              width: 52,
              decoration: BoxDecoration(color: _KB.deleteBg, borderRadius: BorderRadius.circular(10)),
              child: const Icon(Icons.backspace_outlined, color: Color(0xFFFF5252)),
            ),
          ),
          const SizedBox(width: 8),
        ],
        Expanded(
          child: GestureDetector(
            ...
            child: Container(
              height: 44,
              decoration: BoxDecoration(color: _KB.accentBg, borderRadius: BorderRadius.circular(10)),
              child: const Center(
                child: Text('Valider', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
              ),
            ),
          ),
        ),
